...
export const ChatWindow: React.FC<ChatWindowProps> = ({ 
  ...
  const bgStyle = appSettings.wallpaper === 'default' 
    ...
    : { backgroundColor: appSettings.wallpaper };

  const fontSizeClass = appSettings.fontSize === 'small' ? 'text-sm' : appSettings.fontSize === 'large' ? 'text-lg' : 'text-[15px]';
  ...
  const handleDeleteMessages = async () => { if (selectedMessageIds.size === 0) return; setIsDeleting(true); try { const batch = writeBatch(db); selectedMessageIds.forEach(id => { const docRef = doc(db, 'chats', chat.id, 'messages', id); batch.delete(docRef); }); await batch.commit(); setIsSelectionMode(false); setSelectedMessageIds(new Set()); setShowDeleteConfirm(false); } catch (e) { alert("Gagal menghapus pesan"); } finally { setIsDeleting(false); } };
  ...
  return (
    <div className="flex flex-col h-full bg-cream-50 relative pattern-bg">
      ...
      <div className="flex items-center justify-between p-3 border-b border-cream-200 bg-cream-50/95 backdrop-blur-sm z-10 shadow-sm relative">
        {isSelectionMode ? (
          <div className="flex items-center gap-4 w-full">
            <button onClick={toggleSelectionMode} className="p-2 hover:bg-cream-200 rounded-full"><X size={20} className="text-denim-600"/></button>
            <span className="text-lg font-bold text-denim-800">{selectedMessageIds.size} {t.chatWindow.select}</span>
            <div className="flex-1"/>
            <button onClick={() => selectedMessageIds.size > 0 && setShowDeleteConfirm(true)} className="p-2 hover:bg-red-50 text-red-500 rounded-full"><Trash2 size={24} /></button>
          </div>
        ) : (
          <>
            <div className="flex items-center gap-3 cursor-pointer" onClick={handleHeaderClick}>
              ...
              <div className="flex flex-col">
                <h2 className="font-bold text-denim-900 text-[15px] flex items-center gap-1">
                  {resolvedName}
                  {isVerifiedChat && <BadgeCheck size={16} className="text-white fill-blue-500" />}
                </h2>
            ...
            <div className="flex items-center gap-4 text-denim-400 relative">
              <button className="hidden sm:block hover:text-denim-600"><Phone size={20} /></button>
              <button className="hidden sm:block hover:text-denim-600"><Video size={20} /></button>
              <button onClick={() => setShowHeaderMenu(!showHeaderMenu)} className="ps-2 border-s border-cream-300 hover:text-denim-600"><MoreVertical size={20} /></button>
              {showHeaderMenu && (
      ...
      {showContactPicker && (
         <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-denim-900/40 backdrop-blur-sm">
            <div className="bg-white w-full max-w-sm rounded-2xl shadow-xl flex flex-col max-h-[70vh]">
               <div className="p-4 border-b border-cream-200 flex justify-between items-center"><h3 className="font-bold text-denim-900">{t.chatWindow.attach.contact}</h3><button onClick={() => setShowContactPicker(false)}><X size={20} className="text-denim-500"/></button></div>
                  ...
                  {myContacts.map(c => {
                     ...
                     return (
                        <div key={c.id} onClick={() => handleSendContact(c)} className="flex items-center gap-3 p-3 hover:bg-cream-100 rounded-xl cursor-pointer">
                            <img src={isContactAdmin ? adminProfile!.avatar : c.avatar} className="w-10 h-10 rounded-full" />
                            <div>
                                <p className="font-bold text-sm text-denim-900 flex items-center gap-1">
                                    {isContactAdmin ? adminProfile!.name : c.savedName}
                                    {isContactAdmin && <BadgeCheck size={14} className="text-white fill-blue-500" />}
                                </p>
