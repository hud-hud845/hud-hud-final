...
export const ChatWindow: React.FC<ChatWindowProps> = ({ 
  ...
  const renderMessageContent = (msg: Message, isMe: boolean) => {
    ...
    // Check for potential video content in text messages
            ...
            {videoData && (
                ...
                >
                    ...
                    {videoData.type === 'youtube' ? (
                        <img 
                            src={`https://img.youtube.com/vi/${videoData.url.split('/').pop()}/0.jpg`} 
  ...
  return (
    <div className="flex flex-col h-full bg-cream-50 relative pattern-bg">
      ...
      {showContactPicker && (<div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-denim-900/40 backdrop-blur-sm"><div className="bg-white w-full max-w-sm rounded-2xl shadow-xl flex flex-col max-h-[70vh]"><div className="p-4 border-b border-cream-200 flex justify-between items-center bg-cream-50"><h3 className="font-bold text-denim-900">{t.chatWindow.attach.contact}</h3><button onClick={() => setShowContactPicker(false)}><X size={20} className="text-denim-500"/></button></div><div className="flex-1 overflow-y-auto p-2 custom-scrollbar bg-white">{myContacts.map(c => (<div key={c.id} onClick={() => handleSendContact(c)} className="flex items-center gap-3 p-3 hover:bg-cream-100 rounded-xl cursor-pointer"><img src={c.avatar} className="w-10 h-10 rounded-full shrink-0" /><div><p className="font-bold text-sm text-denim-900 flex items-center gap-1">{c.savedName}</p><p className="text-xs text-denim-500">{c.phoneNumber}</p></div></div>))}</div></div></div>)}
    </div>
  );
};
