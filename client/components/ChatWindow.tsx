...
export const ChatWindow: React.FC<ChatWindowProps> = ({ 
      ...
      const fetchMembers = async () => {
        ...
        try {
          for (const uid of chat.participants) {
            ...
            if (adminProfile && uid === adminProfile.id) { members.push(adminProfile); continue; }
            if (contact) { members.push({ id: uid, name: contact.savedName, avatar: contact.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(contact.savedName)}`, phoneNumber: contact.phoneNumber, status: 'offline', bio: t.chatWindow.attach.contact }); } 
            else { 
        ...
        } catch (e) { console.error("Failed fetching members", e); } finally { setLoadingMembers(false); }
      };
      fetchMembers();
    }
  ...
  const handleHeaderClick = async () => { 
    if (chat.type === 'group') { setInfoModalUser(null); setShowInfoModal(true); } 
      ...
      if (partnerId) { 
        if (adminProfile && partnerId === adminProfile.id) { setInfoModalUser(adminProfile); setShowInfoModal(true); } 
        else { await handleOpenUserInfo(partnerId); }
      } 
    } 
  };
  ...
  const handleOpenUserInfo = async (targetUid: string) => {
    ...
    if (contactsMap[targetUid]) { const contact = contactsMap[targetUid]; tempUser = { id: targetUid, name: contact.savedName, phoneNumber: contact.phoneNumber, avatar: contact.avatar, bio: t.chatWindow.attach.contact }; } 
    else if (targetUid === partnerUid && partnerRealtimeData) { tempUser = partnerRealtimeData; }
    if (tempUser && (!tempUser.avatar || tempUser.avatar.includes('ui-avatars.com'))) { tempUser.avatar = `https://ui-avatars.com/api/?name=${encodeURIComponent(tempUser.name || 'User')}&background=random&color=fff`; }
    const docSnap = await getDoc(doc(db, 'users', targetUid)); 
    if (docSnap.exists()) { 
         const userData = docSnap.data();
         let finalAvatar = userData.avatar;
         if (contactsMap[targetUid] && (!finalAvatar || finalAvatar.includes('default'))) finalAvatar = contactsMap[targetUid].avatar;
    ...
    } else if (tempUser) { setInfoModalUser(tempUser as User); }
  ...
  const handleAddUnknownContact = async () => {
      ...
      try {
          await addDoc(collection(db, 'users', currentUser.id, 'contacts'), { uid: infoModalUser.id, savedName: newContactName, phoneNumber: infoModalUser.phoneNumber || '', avatar: infoModalUser.avatar || '' });
          alert("Kontak berhasil disimpan!"); setIsAddingContact(false); setShowInfoModal(false); 
  ...
  // LOGIKA PENGHAPUSAN PERMANEN
  const executeDelete = async (deleteForEveryone: boolean) => {
    ...
    try {
      ...
      idsToDelete.forEach(id => {
        ...
        if (deleteForEveryone && msgDoc.senderId === currentUser.id) {
          // Permanen hapus dari database
          batch.delete(docRef);
        } else {
          // Hapus untuk saya saja (sembunyikan menggunakan array deletedBy)
          batch.update(docRef, {
      ...
      await batch.commit();
      showToast(deleteForEveryone ? "Pesan dihapus untuk semua" : "Pesan dihapus untuk Anda");
    } catch (e) {
      console.error(e);
  ...
  const handleImageSelect = async (e: React.ChangeEvent<HTMLInputElement>) => { if (e.target.files && e.target.files[0]) { const file = e.target.files[0]; setIsUploading(true); setUploadProgress(t.common.processing); try { const url = await uploadImageToCloudinary(file); await handleSendMessage('image', '', { fileUrl: url, fileName: file.name }); } catch (e) { alert("Gagal upload gambar"); } finally { setIsUploading(false); setUploadProgress(''); } } };
  ...
  const renderMessageContent = (msg: Message, isMe: boolean) => {
    if (msg.senderId === 'system') return (<div className="flex justify-center w-full my-1"><div className="bg-cream-200/50 backdrop-blur-sm px-4 py-1.5 rounded-full border border-cream-300"><p className="text-[11px] font-bold text-denim-600 uppercase tracking-tight italic">{msg.content}</p></div></div>);
    switch (msg.type) {
      ...
      case 'document': 
        ...
        return (<div className="flex items-center gap-3 bg-black/5 p-3 rounded-lg min-w-[220px] max-w-[280px]"><div className="shrink-0"><FileText size={28} className="text-red-500" /></div><div className="flex-1 overflow-hidden"><p className="text-sm font-semibold break-words line-clamp-2 leading-tight mb-0.5" title={msg.fileName}>{msg.fileName}</p><p className="text-[10px] opacity-70">{msg.fileSize} • Doc</p></div><button onClick={() => msg.fileUrl && handleDirectDownload(msg.fileUrl, msg.fileName || 'doc', msg.id)} disabled={isDownloading} className="shrink-0 p-2.5 bg-white rounded-full shadow-sm hover:bg-gray-100 transition-colors disabled:opacity-70 flex items-center justify-center h-10 w-10">{isDownloading ? <Loader2 size={18} className="animate-spin text-denim-600" /> : <Download size={18} className="text-denim-600"/>}</button></div>);
  ...
  // FILTER PESAN: Sembunyikan pesan yang sudah dihapus "untuk saya"
  const visibleMessages = messages.filter(msg => !msg.deletedBy?.includes(currentUser.id));

  // CEK APAKAH SEMUA PESAN TERPILIH ADALAH MILIK SAYA
  ...
  return (
    <div className="flex flex-col h-full bg-cream-50 relative pattern-bg">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 z-0 custom-scrollbar relative">
        {visibleMessages.map((msg) => {
          ...
          if (!isMe && chat.type === 'group') {
             ...
             else { senderName = getDisplayName(msg.senderId); if (contactsMap[msg.senderId]) senderAvatar = contactsMap[msg.senderId].avatar; if (!senderAvatar) senderAvatar = `https://ui-avatars.com/api/?name=${encodeURIComponent(senderName)}&background=random&color=fff`; }
          }
      ...
      {/* MODAL KONFIRMASI PENGHAPUSAN DINAMIS */}
      {showDeleteConfirm && (<div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-denim-900/60 backdrop-blur-sm animate-in fade-in"><div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-xl"><div className="w-14 h-14 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-4 text-red-500"><Trash2 size={28} /></div><h3 className="font-bold text-lg text-denim-900 mb-2 text-center">{t.chatWindow.deleteMsg}</h3><p className="text-sm text-denim-500 text-center mb-6">{t.chatWindow.deleteMsgConfirm.replace('{count}', selectedMessageIds.size.toString())}</p><div className="flex flex-col gap-2">
        <button onClick={() => executeDelete(false)} disabled={isDeleting} className="w-full py-3 bg-cream-100 hover:bg-cream-200 text-denim-800 rounded-xl font-bold transition-colors">Hapus untuk Saya</button>
        {allSelectedAreMine && (
          <button onClick={() => executeDelete(true)} disabled={isDeleting} className="w-full py-3 bg-red-500 hover:bg-red-600 text-white rounded-xl font-bold shadow-lg transition-colors flex justify-center items-center gap-2">
            {isDeleting && <Loader2 size={18} className="animate-spin" />}
            Hapus untuk Semua Orang
          </button>
        )}
        <button onClick={() => setShowDeleteConfirm(false)} disabled={isDeleting} className="w-full py-2 text-denim-400 font-medium text-sm mt-1">{t.common.cancel}</button>
      </div></div></div>)}

      {showInfoModal && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-denim-900/60 backdrop-blur-sm animate-in fade-in"><div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl relative animate-in zoom-in-95 flex flex-col max-h-[80vh]"><button onClick={() => setShowInfoModal(false)} className="absolute top-4 right-4 bg-black/20 text-white p-1 rounded-full hover:bg-black/40 z-10 rtl:left-4 rtl:right-auto"><X size={20} /></button><div className="h-32 bg-denim-700 relative rounded-t-2xl shrink-0"><div className="absolute inset-0 opacity-10 pattern-bg rounded-t-2xl"></div></div><div className="px-6 pb-6 -mt-12 flex flex-col items-center relative z-0 flex-1 overflow-y-auto custom-scrollbar"><div className="relative group cursor-pointer" onClick={() => setZoomImage(chat.type === 'group' && !infoModalUser ? chat.avatar : (infoModalUser?.avatar || displayAvatar))}><img src={chat.type === 'group' && !infoModalUser ? chat.avatar : (infoModalUser?.avatar || displayAvatar)} className="w-24 h-24 rounded-full border-4 border-white shadow-md bg-denim-200 object-cover z-10 relative" /></div><h2 className="mt-3 text-xl font-bold text-denim-900 text-center flex items-center justify-center gap-1">{chat.type === 'group' && !infoModalUser ? chat.name : infoModalUser?.name}{((infoModalUser?.isAdmin) || (adminProfile && infoModalUser?.id === adminProfile.id)) && <BadgeCheck size={18} className="text-white fill-blue-500" />}</h2><p className="text-denim-500 text-sm font-medium mb-4 text-center">{chat.type === 'group' && !infoModalUser ? `${chat.participants.length} ${t.groups.members}` : (infoModalUser?.phoneNumber || '-')}</p>{chat.type === 'direct' && infoModalUser && !contactsMap[infoModalUser.id] && (!adminProfile || infoModalUser.id !== adminProfile.id) && (<div className="mb-4 w-full">{!isAddingContact ? (<button onClick={() => { setNewContactName(infoModalUser.name); setIsAddingContact(true); }} className="w-full py-2 bg-denim-600 hover:bg-denim-700 text-white font-bold rounded-lg transition-colors flex items-center justify-center gap-2"><UserPlus size={16} /> Tambahkan Kontak</button>) : (<div className="bg-cream-100 p-3 rounded-xl border border-denim-200"><p className="text-xs text-denim-500 mb-2 font-bold uppercase">Simpan Sebagai:</p><input type="text" value={newContactName} onChange={(e) => setNewContactName(e.target.value)} className="w-full p-2 border border-cream-300 rounded-lg text-sm mb-2 focus:ring-1 focus:ring-denim-500 outline-none" placeholder="Nama Kontak" autoFocus /><div className="flex gap-2"><button onClick={() => setIsAddingContact(false)} className="flex-1 py-1.5 bg-gray-200 text-gray-700 rounded-lg text-xs font-bold">Batal</button><button onClick={handleAddUnknownContact} className="flex-1 py-1.5 bg-green-500 text-white rounded-lg text-xs font-bold">Simpan</button></div></div>)}</div>)}<div className="w-full bg-cream-50 p-4 rounded-xl border border-cream-200 text-center shrink-0"><p className="text-sm text-denim-700 italic">"{chat.type === 'group' && !infoModalUser ? (chat.description || '-') : (infoModalUser?.bio || '-')}"</p></div>{chat.type === 'group' && !infoModalUser && (<div className="mt-4 w-full"><p className="text-xs font-bold text-denim-400 uppercase tracking-wider mb-2 text-center">{t.groups.members}</p><div className="space-y-2 mt-2">{loadingMembers ? <div className="flex justify-center"><Loader2 className="animate-spin text-denim-400" size={20}/></div> : groupMembersInfo.map(m => { const isAdminMember = adminProfile && m.id === adminProfile.id; const memberAvatar = isAdminMember ? adminProfile!.avatar : (m.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(m.name)}`); return (<div key={m.id} className="flex items-center gap-3 p-2 hover:bg-cream-50 rounded-lg"><img src={memberAvatar} className="w-8 h-8 rounded-full object-cover shrink-0"/><div className="text-start flex-1 min-w-0 flex items-center gap-1"><p className="text-sm font-semibold text-denim-800 truncate">{isAdminMember ? adminProfile!.name : m.name}</p>{isAdminMember && <BadgeCheck size={14} className="text-white fill-blue-500" />}</div>{chat.adminIds?.includes(m.id) && <span className="text-[10px] bg-denim-100 text-denim-600 px-2 py-0.5 rounded-full font-bold">{t.chatWindow.admin}</span>}</div>); })}</div></div>)}</div></div></div>)
      )}
      {zoomImage && (<div className="fixed inset-0 z-[100] bg-black flex items-center justify-center p-0 animate-in fade-in duration-200" onClick={() => setZoomImage(null)}><button className="absolute top-4 right-4 text-white/80 hover:text-white z-[101] bg-black/40 rounded-full p-2 rtl:left-4 rtl:right-auto"><X size={28} /></button><img src={zoomImage} className="w-full h-full object-contain pointer-events-none select-none" onClick={(e) => e.stopPropagation()} /></div>)}
      {toastMessage && (<div className="absolute bottom-20 left-1/2 -translate-x-1/2 z-[90] bg-denim-800 text-white px-4 py-3 rounded-xl shadow-2xl animate-in slide-in-from-bottom-5 fade-in duration-300 flex items-center gap-2"><CheckCircle2 size={18} className="text-green-400" /><span className="text-sm font-medium">{toastMessage}</span></div>)}
      {showContactPicker && (<div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-denim-900/40 backdrop-blur-sm"><div className="bg-white w-full max-w-sm rounded-2xl shadow-xl flex flex-col max-h-[70vh]"><div className="p-4 border-b border-cream-200 flex justify-between items-center bg-cream-50"><h3 className="font-bold text-denim-900">{t.chatWindow.attach.contact}</h3><button onClick={() => setShowContactPicker(false)}><X size={20} className="text-denim-500"/></button></div><div className="flex-1 overflow-y-auto p-2 custom-scrollbar bg-white">{myContacts.map(c => { const isContactAdmin = adminProfile && c.uid === adminProfile.id; return (<div key={c.id} onClick={() => handleSendContact(c)} className="flex items-center gap-3 p-3 hover:bg-cream-100 rounded-xl cursor-pointer"><img src={isContactAdmin ? adminProfile!.avatar : c.avatar} className="w-10 h-10 rounded-full shrink-0" /><div><p className="font-bold text-sm text-denim-900 flex items-center gap-1">{isContactAdmin ? adminProfile!.name : c.savedName}{isContactAdmin && <BadgeCheck size={14} className="text-white fill-blue-500" />}</p><p className="text-xs text-denim-500">{c.phoneNumber}</p></div></div>); })}</div></div></div>)}
    </div>
  );
};
